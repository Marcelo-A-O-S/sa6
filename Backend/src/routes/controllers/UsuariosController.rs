use actix_web::{get, post, delete, put, web, web::*, HttpResponse, Responder, Result, Error};
use crate::{entities::Usuarios::Usuario, schema::usuario, services::{TServices::TServices, UsuarioServices::UsuarioServices}};
        ...
        if result_validacao == true {
            if(usuarioBody.Id == 0){
                ...
                match result_buscar{
                    ...
                    Err(err)=>{
                        return Ok((HttpResponse::NotFound().json("Dado não encontrado para atualização")))
                    }
                }
            }
        }else{
    ...
    match result_busca{
        Ok(usuario)=>{
            ...
            match result_delete {
                ...
                Err(err)=>{
                    return Ok(HttpResponse::BadRequest().json("Ocorreu algum erro ao deletar o usuário"));
                }
            }
        }
        Err(err)=>{
